...
export const ItemDetail = ({ item, view }) => {
  console.log("Datos que recibe el componente ItemDetail:", item);
  ...
  const handleAdd = () => {
    if (count > 0) {
      ...
      alert(`Agregaste ${count} unidades de ${item.name} al carrito`);
    } else {
      ...
      <section className="item_detail_container">
        <aside className="item_detail_aside">
          <div className="img-container">
            <img className="img-detail" alt={item.name} src={item.image} />
          </div>
          <p>{item.description}</p>
        </aside>
        <aside className="item_detail_aside_contador">
          <div className="item_info_container">
            <p className="item-tittle-detail">{item.name}</p>
